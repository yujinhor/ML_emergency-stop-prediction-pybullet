...
# --- 0. 시뮬레이션 상수 ---
PHYSICS_TIME_STEP = 1.0 / 240
NUM_EPISODES = 50
MAX_SIM_TIME = 30

# 차량 공기 저항 관련 상수
DRAG_COEFF = 0.82         # 항력 계수 (Cd)
FRONTAL_AREA = 2.7        # 전면 면적 (m^2)
...
def get_random_conditions():
    target_friction = random.choice([1.0, 0.5, 0.1])  #1.0 > 기본 노면, 0.5 > 젖은 노면, 0.1 > 얼어있는 노면
    target_speed = random.uniform(75, 100) # 60에서 80까지의 수를 랜덤 반영 이게 초기속도, km/h라는 정의 필요 > pybulet 은 rad/s 이므로 반영 
    trigger_dist = random.uniform(0.01, 0.5)
    mass = random.uniform(1900, 3100) #현실적인 데이터
    brake_torque = 2500.0 #수정 필요
...
def get_air_density(friction):
    '''왜 범위로 설정??'''
    if friction >= 0.9:      return 1.225  # 기본 노면
    elif friction >= 0.4:    return 1.150  #
    else:                    return 1.350 # 젖은노면
    ...
    for ep in range(NUM_EPISODES):
        ...
        while True:
            ...
            if is_braking_active:
                ...
                for w in wheels:
                    p.setJointMotorControl2(car_id, w, controlMode=p.VELOCITY_CONTROL, targetVelocity=cond['target_speed'], force=3000) # 구동력을 크게 늘림 200> 3000
