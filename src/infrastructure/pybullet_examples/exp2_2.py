...
def set_randomized_physics(robot_id, plane_id):
    ...
    brake_level = random.uniform(4, 8) # 브레이크 레벨 (기존 코드 반영)
...
def control_robot(robot_id, current_pos, current_vel, initial_speed, curve_radius, curve_angle_rad, 
    ...
    if is_braking:
        ...
        # 1. 🚀 (수정) 차량의 '로컬' 전진 속도(Local Y)를 계산
        #    (current_vel[1] (World Y)를 사용하면 조향 시 속도가 0으로 오인됨)
        _ , orn = p.getBasePositionAndOrientation(robot_id)
        # 🚀 current_vel은 이미 함수 인자로 받아옴
        
        rotation_matrix = p.getMatrixFromQuaternion(orn)
        rot_matrix_np = np.array(rotation_matrix).reshape(3, 3)
        inv_rot_matrix_np = rot_matrix_np.T
        local_velocity = inv_rot_matrix_np.dot(current_vel) 
        
        # 🚀 (수정) 이 URDF는 Local Y축이 전진 방향임
        current_forward_speed = local_velocity[1] 
        
        # 2. 목표 속도 (m/s) (약 40km/h)
        target_speed = initial_speed # (11.11 m/s)
        
        # 3. 속도 오차 계산
        speed_error = target_speed - current_forward_speed
        
        # 4. P-제어기 + Feed-Forward로 토크 계산
        Kp_cruise = 10000.0 # 속도 오차에 반응하는 민감도
        base_force = 4000.0 # 🚀 기본 힘 (튜닝 필요, 4000~8000)
        
        total_force = base_force + (Kp_cruise * speed_error)
        total_force = np.clip(total_force, 0, 15000) # (0 미만으로 떨어지지 않게)
...
if __name__ == "__main__":
    ...
    if rnn_all_simulations_data:
        ...
        df.to_csv("nn_model_data.csv2_2", index=False)
        
        print("\nNN 모델용 데이터가 'nn_model_data.csv2_2'에 저장되었습니다.")
        print(df.head())
    else:
