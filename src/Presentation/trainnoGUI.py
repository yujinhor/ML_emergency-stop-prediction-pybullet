...
if __name__ == "__main__":
    ...
    for fname in ["train_summary2.csv", "train_steps2.csv",
                  "val_summary2.csv", "val_steps2.csv"]:
    ...
    for ep in range(NUM_EPISODES):
        ...
        if is_train:
            ...
            df_summary_ep.to_csv(
                "train_summary2.csv",
                mode="a",
                header=not os.path.exists("train_summary2.csv"),
                index=False
            )
            df_steps_ep.to_csv(
                "train_steps2.csv",
                mode="a",
                header=not os.path.exists("train_steps2.csv"),
                index=False
            )
        else:
            val_success_eps += 1
            df_summary_ep.to_csv(
                "val_summary2.csv",
                mode="a",
                header=not os.path.exists("val_summary2.csv"),
                index=False
            )
            df_steps_ep.to_csv(
                "val_steps2.csv",
                mode="a",
                header=not os.path.exists("val_steps2.csv"),
                index=False
            )
